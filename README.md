...
> [!IMPORTANT]
> This project uses pnpm Plug'n'Play as installation strategy for dependencies. [Check out](https://yarnpkg.com/features/pnp) the official pnpm documentation to lean about pnp and its difference from the classic `node_modules` approach.
...
Smart IDEs (such as VSCode or IntelliJ) require special configuration for TypeScript to work when using Plug'n'Play installs. That configuration is generated automatically by `pnpm` (via `pnpm dlx @yarnpkg/sdks vscode vim [other-editor...]`) and commited to `.pnpm/sdks`.
