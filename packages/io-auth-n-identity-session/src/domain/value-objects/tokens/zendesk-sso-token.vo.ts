...
export const toPlainZendeskSSOTokenFromExtended = (
  ...
  PlainZendeskSSOTokenSchema.parse(
    token.slice(0, -EXT_ZENDESK_SSO_TOKEN_SUFFIX_HEX_LENGTH),
  );
...
export const toExtendedPlainZendeskSSOToken = async (
  ...
  const suffix = email
    ? toSha256(email).substring(0, EXT_ZENDESK_SSO_TOKEN_SUFFIX_HEX_LENGTH)
    : await getRandomBytesHex(EXT_ZENDESK_SSO_TOKEN_SUFFIX_BYTES_LENGTH);
