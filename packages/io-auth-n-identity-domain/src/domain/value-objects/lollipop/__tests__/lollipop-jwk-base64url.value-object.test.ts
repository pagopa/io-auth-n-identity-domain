...
import { LollipopJwkSchema } from "../lollipop-jwk.value-object.js";

const toBase64Url = (value: unknown): string =>
  Buffer.from(JSON.stringify(value)).toString("base64url");

describe("LollipopJwkSchema", () => {
  it.each([
    ...
    expect(LollipopJwkSchema.safeParse(base64urlJwk).success).toBe(true);
  });
  ...
  `("should return $scenario", ({ input }) => {
    expect(LollipopJwkSchema.safeParse(input).success).toBe(false);
  });
});
