...
Before concluding that a scenario has no side effects, probe every queue, blob container, and database document the handler touches — do not rely on the HTTP response alone.
...
Normalization functions that recurse only into plain objects silently skip array contents. Any dynamic field nested inside an array element (timestamps, SAS tokens, random IDs) will differ between record and verify runs. Always recurse into array elements.
