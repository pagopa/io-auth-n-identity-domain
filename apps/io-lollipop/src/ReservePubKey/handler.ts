...
export const reserveSingleKey =
  (lollipopPubkeysModel: LolliPOPKeysModel, pubKey: JwkPubKey) =>
  (
...
export const reservePubKeys =
  (
    ...
    eventLogger: Logger,
  ): IReservePubKeyHandler =>
  (inputPubkeys): ReturnType<IReservePubKeyHandler> =>
    pipe(
      getAllAssertionsRef(
        MASTER_HASH_ALGO,
        inputPubkeys.algo,
...
export const getReservePubKeyHandler = (
  ...
  const handler = reservePubKeys(lollipopPubkeysModel, eventLogger);
  const middlewares = [
