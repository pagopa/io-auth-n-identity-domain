...
const mockDataTableProfileEmailsRepository =
  new DataTableProfileEmailsRepository(mockProfileEmailTableClient);

vi.spyOn(mockDataTableProfileEmailsRepository, "insert").mockImplementation(
  () => Promise.resolve(),
);

vi.spyOn(mockDataTableProfileEmailsRepository, "delete").mockImplementation(
  () => Promise.resolve(),
);
...
vi.spyOn(mockProfileModel, "find").mockImplementation(([id, fiscalCode]) =>
  pipe(
    ...
    (foundProfile) =>
      TE.right(O.fromNullable(foundProfile as RetrievedProfile)),
  ),
);
