  ...
  ({ profileModel }) =>
    pipe(
      TE.tryCatch(
        ...
        (err) => toCosmosErrorResponse(err) as CosmosErrors,
      ),
...
const fetchPaginatedProfileVersions: (
  ...
  IResponseSuccessJson<ProfilePageResults>
> = (fiscalCode, page, pageSize) => (deps) =>
  pipe(
    queryProfileVersionsFromDatabase(fiscalCode, page, pageSize)(deps),
    TE.chainW(
      flow(
        processItems,
        TE.mapLeft((_) =>
    ...
    TE.chainW((profiles) =>
      TE.traverseArray(withIsEmailAlreadyTaken(deps.profileEmailReader))(
        profiles,
      ),
    ),
    TE.map((a) =>
...
export function GetProfileVersionsHandler(
  ...
  return async (fiscalCode, page, pageSize) =>
    pipe(
      fetchPaginatedProfileVersions(fiscalCode, page, pageSize),
      RTE.toUnion,
    )(deps)();
}
