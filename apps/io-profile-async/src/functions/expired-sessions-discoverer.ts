...
const createItemToProcess =
  (itemTimeoutInSeconds: number) =>
  (retrievedDbItem: RetrievedSessionNotificationsStrict): ItemToProcess => ({
    retrievedDbItem,
    queuePayload: {
...
const onBadRetrievedItem = (validationErrors: t.Errors): t.Errors => {
  ...
  trackEvent({
    name: "io.citizen-auth.prof-async.expired-sessions-discoverer.permanent.bad-record",
    properties: {
...
const onRevertItemFlagFailure =
  (itemDbSelf: string) =>
  (cosmosError: CosmosErrors): CosmosErrors => {
    trackEvent({
      name: "io.citizen-auth.prof-async.expired-sessions-discoverer.permanent.revert-failure",
      properties: {
        message:
...
const handleQueueInsertFailure: (
  ...
  ({ id, expiredAt, _self }: RetrievedSessionNotificationsStrict) =>
  (queueInsertError: TransientError) =>
...
const sendMessage: (
  ...
  (itemTimeoutInSeconds, queuePayload) => (deps) =>
    pipe(
    ...
    return pipe(
      ...
      RTE.getOrElse((errors) => {
        ...
        if (isLastTimerTriggerRetry(context)) {
          trackEvent({
            name: "io.citizen-auth.prof-async.expired-sessions-discoverer.max-retry-reached",
            properties: {
