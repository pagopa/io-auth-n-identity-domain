...
const deps = {
  sessionNotificationEventsProcessorConfig:
    sessionNotificationEventsProcessorConfigMock,
  sessionNotificationsRepositoryConfig:
    sessionNotificationsRepositoryConfigMock,
  SessionNotificationsRepo:
    SessionNotificationsRepo as unknown as SessionNotificationsRepository,
...
describe("Expired Sessions Discoverer ServiceBusTrigger Tests", () => {
  ...
  describe("Transient and Permanent Errors Handling", () => {
    ...
    it("should not fail on Bad record received querying CosmosDB(only invalid item)", async () => {
      ...
      expect(trackEventMock).toHaveBeenCalledWith({
        name: "io.citizen-auth.prof-async.session-notification-events-processor.permanent.bad-record",
        properties: {
    ...
    it("should fail silently on PermanentError while creating new record(Bad TTL)", async () => {
      ...
      expect(trackEventMock).toHaveBeenCalledOnce();
      expect(trackEventMock).toHaveBeenCalledWith({
        name: "io.citizen-auth.prof-async.session-notification-events-processor.permanent.unable-to-build-new-record",
        properties: {
    ...
    it("should emit a customEvent on TransientError and last retry", async () => {
      ...
      const aLastRetryContextMock = {
        ...serviceBusTriggerContextMock,
        triggerMetadata: {
          ...serviceBusTriggerContextMock.triggerMetadata,
          deliveryCount:
      ...
      expect(trackEventMock).toHaveBeenCalledWith({
        name: "io.citizen-auth.session-notification-events-processor.max-retry-reached",
        properties: {
          errorMessage: expectedErrorMessage,
          message: "Reached max retry for event processing",
          messageId: aLastRetryContextMock.triggerMetadata?.messageId,
        },
        tagOverrides: {
          samplingEnabled: "false",
        },
  ...
  describe("Invalid Event Event Message Processing", () => {
    ...
    it("should fail silently in case of a bad message having an unknown eventType", async () => {
      ...
      expect(trackEventMock).toHaveBeenCalledWith({
        name: "io.citizen-auth.prof-async.session-notification-events-processor.permanent.bad-message",
